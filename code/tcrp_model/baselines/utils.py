...
def load_data_cell_line(tissue_map, drug, tissue_arg, K, path='/home-nfs/wangsheng/GDSC_PDX_WP_feature/'):
	...
	for tissue, cell_line_list in tissue_map.items():
		...
		path_suffix = path + drug + '/' + tissue + '_' + drug
		tissue_feature = np.load(path_suffix + '_feature.npy',allow_pickle=True).astype('float64')
		tissue_label = np.load(path_suffix + '_label.npy',allow_pickle=True).reshape(-1,1).astype('float64')
